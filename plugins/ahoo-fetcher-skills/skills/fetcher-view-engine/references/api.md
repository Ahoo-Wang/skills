...
Package: `@ahoo-wang/fetcher-view-engine`, version `5.0.0`. The filter layer and RecordView are implemented: headless compilation and view engine, definitions/instances with host persistence, and React filter/table/card/page components. AnalysisView includes table, metric, bar, line, area and pie presentations; DashboardView composes cross-definition record/analysis positions, explicit global scopes and saved grid layouts.
...
Composed hosts can also set `DashboardView.title`, `AnalysisTable.label`, and `FilterPanel.ariaLabel` to provide contextual accessible names. Existing standalone defaults are unchanged.
...
For record/analysis positions, `commands.restore()` restores the position’s local baseline without saving the managed instance. Record restore refreshes its own query; analysis restore does not execute a query. Disposal also releases per-position query metadata.

`engine.openPosition(instance, definition)` opens a caller-owned record, analysis or top-level dashboard position after the engine is loaded. It returns a discriminated `kind`, an immutable `identity` (`id`, `instanceId`, `definitionId`), `getSnapshot`, `subscribe`, `commands` for data views or `runtime` for dashboards, and an idempotent `dispose`. Opening does not query; invoke `commands.refresh()` for a record position or `commands.run()` for an analysis position. Analysis `refresh()` retains its existing safe automatic-refresh policy and does not run an unqueried position. Repeated instances receive independent position IDs, pagination, selection and results, and use the provided definition's source.
...
All position sessions remain `dirty: false`; record and analysis sessions expose `positionId`; persisted `instance.id` is unchanged. Positions are not inserted into the managed instance navigation list and cannot be saved through `engine.save(position.identity.id)`. Edit the original managed instance for persistence. Dispose positions when their containing UI closes; commands from a disposed position reject instead of affecting a replacement. Active positions do not consume the managed history-result budget. DashboardRuntime owns orchestration and per-dashboard result budgets; see the dashboard contract below.
...
| `onValidityChange(valid)`              | Reports aggregate editor/buffer validity. Record consumers call `engine.record(id).setFilterValidity(valid)`; reporting true cannot clear an unsubmitted draft.                                  |
...
- `analysis: AnalysisCapability` authorizes COUNT, scalar ANY, predefined Elements scopes, numeric expressions/functions, field grouping, date units and bounds; explicit service `features` gate distinct counts, percentiles, metric filters, derived metrics, HAVING, missing keys and dense date buckets. An aggregate-only source does not need a record row key or paging functions.
...
for aggregation. `engine.record(id).refreshSummary()` recalculates the page and retries aggregation
...
Invalid configuration blocks Save/Save As; valid pending filters can be saved without Query. The
...
Within `RecordView`, cell/card/row renderers receive the executed query configuration, with current presentation settings. Unqueried filter/sort edits do not become the scope of displayed rows.

Definitions select each action area via `record.recordActions.global`, `.toolbar` and `.row`.
...
AnalysisView, record table/card layouts and DashboardView are implemented;
...
`examples/react/FilterPersistenceExample.tsx` and the matching View Engine / 扩展接入 / 公共包 story demonstrate JSON-backed save/new-engine reload of unset controls and opaque selected ID/display label props. Valid working values can be saved without Query. Run `node packages/view-engine/examples/core.mjs` and
...
const definition: ViewDefinition = {
  ...
  title: 'Orders',
  sourceId: 'orders',
  record: { rowKey: 'id', allowedLayouts: ['table', 'card'] },
...
`AnalysisViewConfig` keeps `filters`, `dimensions`, `metrics`, alias-based `sort`, `limit`, optional Elements `scope`, result filter `having` and `presentation`. Each component has a stable `id`, persisted `component` reference, optional `field`, output `alias`, display `title` and raw JSON `props`; metrics may also carry `filters`, ordinary `expression` or `derivedExpression`. Incomplete text can remain in configuration, but compilation returns errors and no executable plan.

`AnalysisCompileContext` supplies filter `fields`, explicit `capability`, optional `timeZone`, `allowedOperators`, `filterCompilers` and custom analysis `compilers`. Capabilities separately authorize COUNT, field grouping, numeric functions and date units. Built-ins are `terms`, `histogram` (`props.interval`), `date-histogram` (`props.unit`), `count`, `any`, `numeric` (`props.function`), `distinct-count`, `percentile` (`props.percentile`) and `derived` (`derivedExpression`, optional `props.displayFormat`). TERMS accepts `props.missingKey`; DATE_HISTOGRAM accepts `props.dense`, subject to the feature and shape constraints below. Date grouping requires an explicit timezone. Custom `AnalysisCompiler` registrations require a nonempty, unique `roles` array (`['dimension']`, `['metric']` or both). `AnalysisComponentCompileContext` extends `AnalysisCompileContext` with the actual readonly `role`; custom compilers and editors can branch on it. Unsupported roles are rejected before compilation. `compile` returns one group or metric, which core validates against its field, alias and capability. The engine and React adapter snapshot and freeze role metadata for each lifecycle.
...
`validateAnalysisResult` returns `{ rows?, errors }`, rejecting the whole result on missing aliases, invalid types, excess rows or duplicate typed dimension tuples. It reads aliases as literal own keys, retains only schema columns, permits null only for nullable columns, and requires COUNT and DISTINCT_COUNT to be nonnegative safe integers. Numeric, percentile and derived columns admit null according to the compiled schema. Date bucket values are epoch milliseconds. Call `analysisRowKey(row, plan.schema.filter(column => column.role === 'dimension'))` on a validated row for its typed dimension identity.

### Aggregation features and metric configuration
...
| Derived metric `props.displayFormat`               | `number` (default) or `percent`. Percent requires proven dimensionless units (constants, counts, or a ratio of matching known units); unknown or incompatible units are rejected. Intl formatting does not modify the raw value. |
...
`AnalysisCapability.scopes` contains host-authorized `AnalysisScopeDefinition` entries: `{ id, label, elements: [{ path, fields }], fields, capability }`. `AnalysisViewConfig.scope` selects `{ id, filters }`, with exactly one `FilterConfiguration` per element. Each element path is relative to the preceding scope; its filter fields are relative to the current element. Final grouping/metric fields use the scope's `fields` and capability, inheriting service features and request limits from the root. Root `config.filters` always uses the original root fields. Chains contain at most five elements. `analysisScopeContext(config, context)` resolves the selected field/capability context and throws for an unknown or duplicate scope ID. Switching scope in the editor preserves existing query drafts for repair.

`capability.expressions: true` enables an explicit `component.expression` on numeric, distinct-count and percentile metrics; omit `component.field` in expression mode. `AnalysisNumericExpression` is a `FIELD` (`field`), `CONSTANT` (`value: number | string`) or `BINARY` (`operator`, `left`, `right`) tree using Wow enum values. Operators are ADD, SUBTRACT, MULTIPLY and DIVIDE. `compileAnalysisExpression(expression, function, context)` validates finite constants, each numeric field/function authorization, depth ≤ 8 and nodes ≤ 256. Text such as `1e` remains in the draft but cannot run. Field mode keeps the existing `field` + `props.function` contract; histogram intervals also retain raw editor text.

The `any` component requires an authorized scalar `field` with `capability.fields[].any: true`. It returns a representative value, not a stable group key or a numeric chart metric. COUNT, DISTINCT_COUNT, SUM, AVG, MIN, MAX, STDDEV, VARIANCE, PERCENTILE, DERIVED and ANY semantics are recorded in `AnalysisResultColumn.aggregation`; dimensions carry `group` metadata with type and applicable interval, unit and timeZone.

### Wow Schema mapping

`adaptWowAnalysisSchema(schema, options?)` is a pure adapter returning `{ model, fields, capability }`; it does not fetch Schema or create a client. `WowAnalysisSchemaOptions` accepts `labels` and `units` keyed by absolute logical field paths, plus explicit service `features`. It forwards host features without probing backend support; schema field capabilities remain independently required. Supply the returned fields/capability to the definition, then use the existing Snapshot/EventStream query client as the host's aggregation source and forward cancellation.
...
`AnalysisEditor.appliedValue` optionally supplies the last successfully executed configuration as the element- and metric-filter undo baseline (only matching scope IDs apply; metrics also match component IDs). `AnalysisView` uses the retained successful execution for root and element filter undo, falling back to the saved configuration before the first result. Analysis component IDs must be globally unique across dimensions and metrics at instance admission. Delayed custom-editor changes target the latest item by ID and are ignored after removal, component replacement, disablement or unmount. Builtin group choices intersect declared capabilities with field types and require an explicit time zone and allowed date units for date histograms.

`AnalysisEditor` also accepts `filterContext`, combined filter/analysis `extensions`, and `onFilterValidityChange(valid)` for the current element- and metric-filter editors. `AnalysisView` combines root, element and metric editor validity before Run/Save; a valid panel cannot clear another panel's invalid buffer. Scope and instance changes discard obsolete editor validity. Custom scope filter components receive the same extensions and host context as root filters.

`DialogContent.keepMounted?: boolean` defaults to `false` and forwards Base UI Portal retention. Retained popup content remains hidden while closed. AnalysisView uses one stable editor portal target across desktop/dialog placements so extension-local input and validity survive resizing or closing. Pie/donut charts show raw formatted values with returned-group shares; ratios are normalized before summing, avoiding overflow of otherwise finite values.

`AnalysisEditor.visible?: boolean` defaults to visible. Owners retaining a hidden editor set it to `false` to dismiss floating controls without discarding drafts. Analysis uses three regions: a right-side query Sheet, central results, and a left visualization panel collapsed by default. The query Sheet keeps its editor mounted across closing and resizing; closing preserves drafts and neither saves nor runs. Select a chart type before configuring visualization fields. Unconfigured analysis defaults to the data table; saved charts retain their presentation. The bottom Analysis / Table modes are always available, even without a chart or result, and switching does not query or save. Chart incompatibility stays in Analysis mode with an explicit Table action. Dimensions and metrics keep their summary chips, retained popover editors and keyboard ordering. `AnalysisView.configurationOpen` controls only the query Sheet. `AnalysisPresentationEditor.chartOnly` defaults to false; AnalysisView enables it so table inspection belongs to the bottom mode switch and chart selection uses five icon/name radio cards with no default selection.
...
AnalysisView shares refresh controls and in-place expansion with RecordView. Automatic intervals are off/30s/1m/5m, pause on unrun query changes, invalid filters, writes, reload requirements, errors, loading, background visibility or editor focus, and stop on unmount. Manual refresh is disabled for unrun query changes. Expansion uses the inherited ViewPage container, or the standalone analysis root, and Escape restores the original layout without remounting.

Analysis element expansion chains are limited to five layers at schema adaptation, definition admission, and query compilation, matching the Wow backend contract. Loading an uncached instance retains the selected workspace and its query until the target is successfully admitted; failed or superseded loads do not clear the workspace. Root and element analysis filter summaries use the same per-node configuration/option-label resolution as record filters.

`MAX_ANALYSIS_ELEMENTS` is a runtime export from the core entry and equals `5`. Analysis output titles (dimensions, metrics and dimension labels) must contain non-whitespace text to compile; structural admission still retains incomplete drafts. Formula aggregation choices are restricted to functions accepted by the expression compiler for its current fields. Result-table query actions obey the complete session query validity, including configuration size limits.

`engine.analysis(id).setSort(sort)` is the query-producing result-sort action. It checks current manual-query eligibility, validates the complete proposed configuration (including size and effective sort capacity), and only then publishes the configuration and runs. Invalid current input is a no-op; invalid proposed sorting rejects without editing or querying. Editor `edit` and `clearSort` remain draft-only. `setSort` observes editor-epoch resets like `edit`. Shared internal defaults, group compatibility and effective sort counting are consumed by admission/compiler and controls; UI does not own these rules.
...
Visualization selection uses five icon/name cards: metric, bar, line, area and pie. All remain visible; unavailable types include reasons. Availability means at least one legal mapping exists over the last successful result schema and rows, not that a default mapping happens to work. Pure capability inference checks all supported axis/single-measure candidates through the existing projection validator; additional measures cannot repair a failing single-measure mapping. Recommendation is deterministic: ungrouped metrics, otherwise time trends, otherwise category comparisons. It never selects a type. The chosen type exposes data mappings below the cards, followed by collapsed style settings; no wizard or business phase is stored. Ambiguous mappings remain unselected. A saved type or selected measure that becomes incompatible remains visible and unchanged until explicit repair. Standalone editors without `chartOnly` retain the general presentation selector including table; the analysis workbench excludes table from the cards because inspection belongs to the bottom mode switch.
...
The packed core and React entries are verified under Bundler and NodeNext with strict declaration checking (`skipLibCheck: false`). Negative probes verify that an empty aggregation metrics list and missing `useViewEngine` scope remain type errors. Run `node packages/view-engine/scripts/verify-package.mjs` after building the package and its dependencies.
